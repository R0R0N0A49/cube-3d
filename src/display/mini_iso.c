...
/*   Updated: 2024/11/08 10:23:41 by derey            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	init_corners(t_map *data, int width, int height)
{
	...
	data->m_map->corners[0][1] = data->m_map->mini_y - height / 2;
	data->m_map->corners[1][0] = data->m_map->mini_x + (width / 2);
	data->m_map->corners[1][1] = data->m_map->mini_y;
	data->m_map->corners[2][0] = data->m_map->mini_x;
	data->m_map->corners[2][1] = data->m_map->mini_y + height / 2;
	data->m_map->corners[3][0] = data->m_map->mini_x - (width / 2);
...
void	draw_minimap(t_map *data)
{
	...
	data->m_map->mini_y = (int)(data->m_map->iso_y
			* MINIMAP_SCALE) + MINIMAP_OFFSET_Y;
	draw_isometric_rhombus(10, 10, PLAYER_COLOR, data);
}
