...
/*   Updated: 2024/11/16 11:27:17 by trebours         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	if (door->isopen && door->isanime && door->index < 13 && get_time() - door->time > 50)
		door->index++;
	else if (!door->isopen && door->dist_center > 2 && door->index > 0 && get_time() - door->time > 50)
	{
		door->index--;
		door->isanime = true;
	}
...
static void	next(t_door *door, int *result, int max)
{
	for (int i = 0; i < max; i++)
	{
		result[i] = i;
...
void	chois_door(t_map *data, t_ray *ray)
{
	...
	(void)ray;
	i = 0;
	next(data->door, j, data->nmb_door);
