...
/*   Updated: 2024/09/28 17:27:39 by derey            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	init_pos(t_map *data, int j, int i)
{
	data->game->plane_x = 0;
	data->game->plane_y = 0;
	data->game->dir_x = 0;
...
void	init_edit(t_map *data)
{
	data->edit = mlx_new_image(data->mlx, WINDOWSW, WINDOWSH);
	data->editor->txt_edit = mlx_load_png("./tiles/menufd2.png");
	data->editor->edit = false;
	data->editor->good = true;
	data->edit = mlx_texture_to_image(data->mlx, data->editor->txt_edit);
	mlx_image_to_window(data->mlx, data->edit, 0, 0);
}

void	init_menu(t_map *data)
{
...
void	init_option(t_map *data)
{
	...
	data->opt->but_music->click = false;
	data->opt->but_fov->click = false;
	data->opt->but_floor->click = false;
	data->opt->but_roof->click = false;
...
void	init_button_menu(t_map *data)
{
	t_button play;
	t_button option_;
	t_button edit;
...
void	init_button_option(t_map *data)
{
	t_button rtn;
	t_button music;
	t_button fov;
...
void	free_texture(t_map *data)
{
	mlx_delete_texture(data->cub);
	mlx_delete_texture(data->texplay);
	mlx_delete_texture(data->texedi);
	mlx_delete_texture(data->texopt);
	mlx_delete_texture(data->texexit);
	mlx_delete_texture(data->nuit);
	mlx_delete_texture(data->menufd2);
	mlx_delete_texture(data->menufd);
	//mlx_delete_texture(data->opt->fov_txt);
	mlx_delete_texture(data->opt->unvalid_txt);
	mlx_delete_texture(data->opt->rtn_txt);
	//mlx_delete_texture(data->opt->roof_txt);
	//mlx_delete_texture(data->opt->floor_txt);
	mlx_delete_texture(data->opt->music_txt);
	mlx_delete_texture(data->opt->valid_txt);
}

void	free_image(t_map *data)
{
	mlx_delete_image(data->mlx, data->cubd);
	mlx_delete_image(data->mlx, data->img_play);
	mlx_delete_image(data->mlx, data->img_edit);
	mlx_delete_image(data->mlx, data->img_option);
	mlx_delete_image(data->mlx, data->img_exit);
	mlx_delete_image(data->mlx, data->edit);
	mlx_delete_image(data->mlx, data->rayc);
	mlx_delete_image(data->mlx, data->minima);
	mlx_delete_image(data->mlx, data->menu);
	mlx_delete_image(data->mlx, data->opt->bottom);
	mlx_delete_image(data->mlx, data->opt->cub);
	mlx_delete_image(data->mlx, data->opt->rtn);
	mlx_delete_image(data->mlx, data->opt->m_valid);
	mlx_delete_image(data->mlx, data->opt->f_valid);
	mlx_delete_image(data->mlx, data->opt->fl_valid);
	mlx_delete_image(data->mlx, data->opt->r_valid);
	mlx_delete_image(data->mlx, data->opt->m_unvalid);
	mlx_delete_image(data->mlx, data->opt->f_unvalid);
	mlx_delete_image(data->mlx, data->opt->fl_unvalid);
	mlx_delete_image(data->mlx, data->opt->r_unvalid);
	mlx_delete_image(data->mlx, data->opt->music);
	//mlx_delete_image(data->mlx, data->opt->fov);
	//mlx_delete_image(data->mlx, data->opt->floor);
	//mlx_delete_image(data->mlx, data->opt->roof);
}
